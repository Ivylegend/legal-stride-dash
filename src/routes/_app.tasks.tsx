...
interface TaskRow { publicId?: string; name?: string; dueDate?: string; status?: string; assignedUsers?: unknown[]; litigation?: unknown; [k: string]: unknown }

function TasksPage() {
  const [q, setQ] = useState("");
  const [status, setStatus] = useState("all");
  const query = useQuery({ queryKey: ["tasks"], queryFn: async () => unwrapList<TaskRow>(await api("/task")) });
  const rows = useMemo(() => {
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <DataPane
          ...
          empty={<EmptyState icon={<CheckSquare className="h-6 w-6" />} title="No tasks yet" description="Create tasks to keep your matters on schedule." />}
        >
          {(list) => (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {list.map((r, i) => (
                  <TableRow key={(r.publicId as string) ?? i}>
                    ...
                    <TableCell>{Array.isArray(r.assignedUsers) && r.assignedUsers.length ? `${r.assignedUsers.length} user(s)` : "Unassigned"}</TableCell>
