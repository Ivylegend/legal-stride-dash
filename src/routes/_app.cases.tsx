...
interface CaseRow { publicId?: string; suitNumber?: string; suitName?: string; type?: string; filingDate?: string; court?: string; status?: string; createdAt?: string; [k: string]: unknown; }
...
function CasesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const matchQ = !s || [r.suitNumber, r.suitName, r.court, r.type].some((v) => (v ?? "").toString().toLowerCase().includes(s));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search by suit number, name or court…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
        </div>
        ...
        <DataPane
          ...
          empty={
            <EmptyState
              ...
              action={<Button onClick={() => setOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> New case</Button>}
            />
          }
        >
          {(rows) => (
              ...
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={(r.publicId as string) ?? i}>
                    ...
                    <TableCell className="font-medium">{r.suitName ?? pick<string>(r, "title", "name") ?? "—"}</TableCell>
